defmodule Tortoise311.Handler do
  @moduledoc """
  User defined callback module for handling connection life cycle events.

  `Tortoise311.Handler` defines a behaviour which can be given to a
  `Tortoise311.Connection`. This allow the user to implement
  ...
  connection, the provided callbacks are:
    ...
    - `init/1` and `terminate/2` are called when the connection is
      ...
      `Tortoise311` will attempt to reconnect, and events that should
  ...
  process using the `Tortoise311.subscribe/3` and
  `Tortoise311.unsubscribe/3` it is possible to make changes to the
  ...
  `Tortoise311`-module are for the most part blocking operations, or
  ...
  arguments and the module driving the handler. This allow Tortoise311 to
  ...
  defmacro __using__(_opts) do
    quote location: :keep do
      ...
      defoverridable Tortoise311.Handler
    end
  end
